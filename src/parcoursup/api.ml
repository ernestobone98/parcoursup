...
let ajoute_voeu session ~rang_repondeur ~nom_candidat ~nom_formation = 
  ...
  | hd::tl -> 
    ...
    (* let ajoute_voeu session ~rang_repondeur ~nom_candidat ~nom_formation = 
      let rec aux (v: voeu list) = match v with 
      | [] -> session.voeu <- [{rang_repondeur=rang_repondeur ;nom_candidat=nom_candidat ; nom_formation = nom_formation }]
      | [x] when x.nom_candidat = nom_candidat && x.nom_formation = nom_formation -> 
          session.voeu <- [{rang_repondeur=rang_repondeur ;nom_candidat=nom_candidat ; nom_formation = nom_formation }]
      | hd::tl -> 
          aux tl ;
          session.voeu <- [hd] @ tl
    ...
    (* let ajoute_voeu session ~rang_repondeur ~nom_candidat ~nom_formation = 
      let rec aux (v: voeu list) = match v with 
      | [] -> [{rang_repondeur=rang_repondeur ;nom_candidat=nom_candidat ; nom_formation = nom_formation }]
      | [x] when x.nom_candidat = nom_candidat && x.nom_formation = nom_formation -> 
          [{rang_repondeur=rang_repondeur ;nom_candidat=nom_candidat ; nom_formation = nom_formation }]
      | hd::tl -> 
          [hd] @ aux tl
...
(* let ajoute_voeu session ~rang_repondeur ~nom_candidat ~nom_formation = 
  let rec aux (v: voeu list) = match v with 
  | [] -> session.voeu <- [{rang_repondeur=rang_repondeur ;nom_candidat=nom_candidat ; nom_formation = nom_formation }]
  | [x] when x.nom_candidat = nom_candidat && x.nom_formation = nom_formation && rang_repondeur > x.rang_repondeur -> 
      session.voeu <- [{rang_repondeur=rang_repondeur ;nom_candidat=nom_candidat ; nom_formation = nom_formation }]
  | hd::tl -> 
      session.voeu <- session.voeu @ [hd] @ aux tl 
    in aux session.voeu  *)

    (* type session = char (* <- modifier *)
